...
export default class Fetch extends Component {
  ...
  componentDidMount(){
    fetch(`https://api.themoviedb.org/3/movie/now_playing?api_key=da46474f8dd8ea1caebd07b84cb98b86&language=en-US&page=1`).then(res => {
      return res.json();
    }).then((dt) => {
      this.setState({
        data: dt
  ...
  getContact = () => {
    if (this.state.data != null) {
      ...
      this.state.data.results.map((item) => { 
        let image = <div className="module" key={item.title}>
          ...
          <input type="Button" defaultValue="More Details" className="button" ></input>
